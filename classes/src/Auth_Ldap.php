...
class Auth_Ldap {
    ...
    public static function ldap_escape($str, $for_dn = false) {
        ...
        $_str = str_replace($metaChars, $quotedMetaChars, $str); //replace them
        return ($_str);
    }
     ...
     * @return mixed
     */
    public static function auth($sn, $password) {
        ...
        if ($ds) {
            ...
            if ($conf_auth['ldap']['anonymus_bind'] == 1) {
                ...
                $dn = $conf_auth['campo_nick'] . "=" . self::ldap_escape($conf_auth['bind_user']);
            ...
            $filter = "(" . $conf_auth['campo_nick'] . "=" . self::ldap_escape($sn) . ")";


            // 1o test con uid
            $sr = @ldap_search($ds, $conf_auth['ldap']['base_dn'], $filter) or 
                    die(openErrorGenerico(_("Could not connect to LDAP server") . ": " . __LINE__));
