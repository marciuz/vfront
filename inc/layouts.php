...
// SMISTA I FILES CSS E JS MANDATI ALLA FUNZIONE	
    ...
    $THEME = (isset($_SESSION['VF_VARS']['layout']) && is_dir(FRONT_ROOT . "/themes/" . $_SESSION['VF_VARS']['layout'] . "/")) ? $_SESSION['VF_VARS']['layout'] : 'default';
...
// LAYOUT 
    ...
    if ($tipo != "sottomaschera" && $tipo != 'popup' && $tipo != 'popup1'){
        ...
        $OUTMENU = (User_Session::is_logged()) ? menu(User_Session::level()) : "";
...
function closeLayout1($back = "", $no_identity = false) {
    ...
    if (User_Session::is_logged()){
        $nome_visualizzato = (!empty(User_Session::fullname())) ? User_Session::fullname() : User_Session::email();
        $OUTPUT_CLOSE .= ($no_identity) ? '' : "<div id=\"identita\">" . _("Logged in as") . " <br /><a href=\"" . FRONT_DOCROOT . "/dati_personali.php\">" . $nome_visualizzato . "</a> (" . _('group') . ":" . Common::gid2group_name(User_Session::gid()) . ")</div>\n";
        }
...
function breadcrumbs($array, $separator = " &raquo; ") {
    ...
    foreach ($array as $url => $name) {
        ...
        if ($name == 'HOME'){
            ...
            $OUT .= ($c == count($array)) ? _('administration') : "<a href=\"" . FRONT_DOCROOT . "/admin/\">" . _('administration') . "</a>";
                }
        else{
