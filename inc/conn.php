...
//############################################################################
// Linux or windows ?
$dir_vfront = (substr(realpath(__FILE__), 0, 1) == '/') ? str_replace("/inc/conn.php", '', realpath(__FILE__)) : str_replace("\\inc\\conn.php", '', realpath(__FILE__));
...
elseif ($db1['dbtype'] == 'sqlite') {
    ...
    if (class_exists('SQLite3') && $db1['dbsqlite_version'] == '3') {
        ...
        die(_('The SQLite database seems to be incompatible with the installed libraries'));
    }
}
else {
 ...
 * Mediante questa funzione, richiamata in testa ad ogni pagina, è possibile proteggere il singolo script 
...
function proteggi($livello = 1) {
    ...
    if ($livello > 0) {
        ...
        elseif (User_Session::level() < $livello) {
