...
class Auth_DB_Ext {
    ...
    public static function auth($user, $passw) {
        ...
        $sql_auth = sprintf("SELECT * FROM {$conf_auth['db_ext']['tabella']} 
                ...
                self::personal_hash_db_ext(trim($passw))
        );
        ...
        if ($conf_auth['db_ext']['dbtype'] == 'mysql') {
            ...
            if (function_exists('mysqli_connect')) {
                ...
                die(sprintf(_("Could not connect to %s server for authentication: php extension missing"), $conf_auth['db_ext']['dbtype']));
        }
     ...
     * @return string
     */
    public static function personal_hash_db_ext($passw) {
