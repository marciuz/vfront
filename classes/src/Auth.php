...
class Auth {
    ...
    public function __construct($user, $passw, $urlreq = '') {
        ...
        switch ($this->tipo_external_auth) {
            ...
            case 'soap':
                $this->auth_obj = Auth_Soap::auth($this->user, $this->passw);
                $this->modalita_auth = '2step';
                break;

            case 'db':
                $this->auth_obj = Auth_DB::auth($this->user, $this->passw);
                $this->modalita_auth = '2step';
                break;

            case 'db_ext':
                $this->auth_obj = Auth_DB_Ext::auth($this->user, $this->passw);
                $this->modalita_auth = '2step';
                break;
...
//-- Fine funzione AUTH
    ...
    function __step_2() {
        ...
        // Prendo i dati a disposizioni e creo un record nel DB con livello di accesso default
            ...
            $DEFAULT_GROUP_EXT = (isset($_SESSION['VF_VARS']['default_group_ext'])) ? intval($_SESSION['VF_VARS']['default_group_ext']) : 0;
    ...
    function __respingi_utente($tipo_err = 0) {
        ...
        switch ($tipo_err) {
            ...
            case 1: // livello troppo basso per vedere la pagina
                header("Location: " . FRONT_DOCROOT . "/index.php?accesso_vietato=1&urlreq=" . $this->urlreq);
                exit;
                break;
        }
    }
